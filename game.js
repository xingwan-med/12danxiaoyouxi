// 🎮 **完整版游戏** - 12种子弹射击游戏 + 音效系统
console.log('🚀 启动完整版游戏 - 包含12种子弹和音效系统');
...
class SoundManager {
    ...
    toggle() {
        ...
        if (btn) {
            ...
            btn.className = this.enabled ? 'sound-btn on' : 'sound-btn off';
        }
        console.log(this.enabled ? '🔊 音效已开启' : '🔇 音效已关闭');
    }
}
...
// ==================== 音效控制按钮事件 ====================
document.getElementById('soundToggleBtn').addEventListener('click', function(e) {
    e.preventDefault();
    e.stopPropagation();
    soundManager.toggle();
    this.style.transform = 'scale(0.9)';
...
console.log('🔊 音效系统已启用：射击、击中、爆炸、切换都有音效！');
